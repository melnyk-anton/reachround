...
import { ArrowLeft, CheckCircle2, XCircle, Edit2, Save, Mail, Loader2 } from 'lucide-react'
...
export default function CampaignEmailQueuePage() {
  ...
  const sendApprovedEmails = async () => {
    ...
    setSending(true)
    try {
      const response = await fetch(`/api/campaigns/${campaignId}/emails/send-approved`, {
        method: 'POST',
      })
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button
                variant="ghost"
                size="sm"
                onClick={() => router.push(`/dashboard/${projectId}/campaigns/${campaignId}`)}
              >
            ...
            {stats.approved > 0 && (
              <Button onClick={sendApprovedEmails} disabled={sending} className="gap-2">
                {sending ? (
                  <>
                ...
                <Card key={email.id} className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      {email.investor.firm && (
                        <p className="text-sm text-muted-foreground">{email.investor.firm}</p>
                      )}
                      {email.investor.email && (
                        <p className="text-sm text-muted-foreground">{email.investor.email}</p>
                      )}
                    </div>
